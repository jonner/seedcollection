...
use crate::{
    cli::{AdminCommands, DatabaseCommands, GerminationCommands, UserCommands},
...
pub(crate) async fn handle_command(dbpath: Option<PathBuf>, command: AdminCommands) -> Result<()> {
    match command {
        AdminCommands::Users { command } => handle_user_commands(dbpath, command).await,
        AdminCommands::Germination { command } => {
            handle_germination_commands(dbpath, command).await
        }
        AdminCommands::Database { command } => handle_database_commands(dbpath, command).await,
    }
}

async fn handle_database_commands(
...
async fn handle_germination_commands(
    dbpath: Option<PathBuf>,
...
async fn handle_user_commands(dbpath: Option<PathBuf>, command: UserCommands) -> Result<()> {
    let db = Database::open(dbpath.ok_or_else(|| anyhow!("No database specified"))?).await?;
    match command {
