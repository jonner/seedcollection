...
def find_genus_synonym(cursor, genus):
    ...
    res = cursor.execute('''SELECT S.tsn_accepted as tsn from taxonomic_units T
    ...
    debug_row(row)
    if row:
        logging.info("Found synonym {}, looking up info about it"
                     .format(row['tsn']))
        res = cursor.execute('''SELECT T.tsn, T.unit_name1 as genus, T.rank_id
...
def find_synonym(cursor, name1, name2, name3, rank):
    ...
    if row:
        logging.info("Found synonym {}, looking up info about it"
                     .format(row['tsn']))
        res = cursor.execute('''SELECT T.tsn, T.complete_name,
...
def find_possibilities(cursor, name1, name2, name3, rank):
    logging.info("Looking for other possibilities for {} {}"
                 .format(name1, name2))
    if rank == RANK_SPECIES:
        res = cursor.execute('''SELECT * from taxonomic_units T
    ...
    if res is not None:
        ...
        if not rows:
            return False
        logging.warning("Unable to find an exact match for {}. Set DEBUG=1 to view {} possible matches"
                        .format((name1, name2, name3, rank), len(rows)))
...
def get_taxon(cursor, name1, name2, name3, rank):
    synonym = False
    dname = displayname(name1, name2, name3)
    logging.info("Looking up information for {}"
                 .format((name1, name2, name3, rank)))
    res = None
    if rank == RANK_SPECIES:
        res = cursor.execute('''SELECT T.tsn, rank_id, complete_name,
    ...
    if row is not None:
        ...
        logging.info("{}{} is <{}> {} ({})"
                     .format(prefix, dname, row['tsn'],
                             row['complete_name'], cname))
        return row['tsn']
    return None
...
def handle_taxa_list(cursor, reader):
    ...
    for row in reader:
        ...
        if new_genus:
            logging.info("genus {} is a synonym for {}, using new name {} {}"
                         .format(name1, new_genus, new_genus, name2))
        ...
        if not find_possibilities(cursor, name1, name2, name3, rank):
            logging.warning("unable to find species {} {}"
                            .format(name1, name2))
    return taxa
...
def check_fieldnames(fieldnames):
    if (len(reader.fieldnames)) != len(CSV_FIELDS):
        raise RuntimeError("Expected 9 fields, found {}"
                           .format(len(reader.fieldnames)))
    for i in range(len(fieldnames)):
        if CSV_FIELDS[i] != fieldnames[i]:
            raise RuntimeError("Field name mismatch. expected field named '{}' in column {}, found '{}'"
                               .format(CSV_FIELDS[i], i, fieldnames[i]))
