...
def find_genus_synonym(cursor, genus):
    ...
    if row:
        ...
        res = cursor.execute(
            'SELECT T.tsn, T.unit_name1 as genus, T.rank_id FROM taxonomic_units T \
...
def find_synonym(cursor, name1, name2, name3, rank):
    ...
    if rank == RANK_SPECIES:
        ...
        res = cursor.execute(
            ...
            INNER JOIN synonym_links S ON T.tsn=S.tsn \
            WHERE name_usage="not accepted" AND unit_name1=? and unit_name2=? \
            AND unit_name3=? AND kingdom_id=? and rank_id=?',
...
def handle_taxa_list(cursor, reader):
    ...
    for row in reader:
        ...
        if not find_possibilities(cursor, name1, name2, name3, rank):
            logging.warning(
                "unable to find species {} {} {}".format(name1, name2, name3)
            )
    return taxa
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--updatedb",
        action="store_true",
        help="Update the mntaxa table with the output from the script",
    )
    ...
    if taxa and args.updatedb:
        ...
        cursor.executemany(
            "INSERT INTO mntaxa ('tsn', 'native_status') VALUES (?, ?)", taxa.items()
        )
        dbconn.commit()
